...
function processModuleComment(comment, moduleName) {
    var md = "";
    var name = moduleName || determineTagValue(TAG_NAMES.MODULE, comment) || settings.get("defaultModuleHeading");
        ...
        for (var tagName in tags[i]) { // I wish I knew another in-built way to get hold of the key in a key/value pair
            switch (tagName) {
                case TAG_NAMES.PARAM:
                    if (!paramsStarted) {
                        md += applyMarkdown(settings.get("paramsHeading"),
                ...
                case TAG_NAMES.RETURN:
                    md += "\n" + applyMarkdown(settings.get("returnHeading"), settings.get("returnHeadingMarkdown")) +
                        "\n\n";
