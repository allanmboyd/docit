...
var apiTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.API_TAG.TOKEN);
};

var classTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.CLASS_TAG.TOKEN);
};
...
var methodTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.METHOD_TAG.TOKEN);
};

var moduleTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.MODULE_TAG.TOKEN);
};
...
var inlineCommentProcessor = function (commentLine) {
    if (state === STATES.IN_COMMENT) {
        commentLine = removePrecedingAsterisks(commentLine).trim();
    }

    return commentLine;
};

var paramTagProcessor = function (commentLine) {
    return parseParameterOrReturnTag(commentLine, TOKEN_PROCESSORS.PARAM_TAG.TOKEN);
};

var privateTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.PRIVATE_TAG.TOKEN);
};

var returnTagProcessor = function(commentLine) {
    return parseParameterOrReturnTag(commentLine, TOKEN_PROCESSORS.RETURN_TAG.TOKEN);
};

var requiresTagProcessor = function(commentLine) {
    return processSimpleTagValue(commentLine, TOKEN_PROCESSORS.REQUIRES_TAG.TOKEN);
};
...
var TOKEN_PROCESSORS = {
    ...
    API_TAG: {
        ...
        PROCESSOR: apiTagProcessor
    },
    CLASS_TAG: {
        TOKEN: exports.TAG_NAMES.CLASS,
        PROCESSOR: classTagProcessor
    },
    METHOD_TAG: {
        TOKEN: exports.TAG_NAMES.METHOD,
        PROCESSOR: methodTagProcessor
    },
    MODULE_TAG: {
        TOKEN: exports.TAG_NAMES.MODULE,
        PROCESSOR: moduleTagProcessor
    },
    PARAM_TAG: {
        TOKEN: exports.TAG_NAMES.PARAM,
        PROCESSOR: paramTagProcessor
    },
    PRIVATE_TAG: {
        TOKEN: "@private", // todo process private properly and allow it to be configurable
        PROCESSOR: privateTagProcessor
    },
    RETURN_TAG: {
        TOKEN: exports.TAG_NAMES.RETURN,
        PROCESSOR: returnTagProcessor
    },
    REQUIRES_TAG: {
        TOKEN: exports.TAG_NAMES.REQUIRES,
        PROCESSOR: requiresTagProcessor
    }
};
    ...
    var augmentedLines = lines.map(function(line) {
        ...
        for (var token in TOKEN_PROCESSORS) {
            if (TOKEN_PROCESSORS.hasOwnProperty(token) && newLine.indexOf(TOKEN_PROCESSORS[token].TOKEN) !== -1) {
                newLine = TOKEN_PROCESSORS[token].PROCESSOR(newLine);
...
function parseParameterOrReturnTag(paramCommentLine, tagToken) {
    var parts = {};
...
function removePrecedingAsterisks(line) {
    ...
    return match ? line.trim().substr(match.index) : "";
}
...
function processSimpleTagValue(commentLine, tagToken) {
    var tag = {};
