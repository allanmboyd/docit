 ...
 * starts with a comment then null is returned. If there are no comments then the commentedCode is returned.
 *
 * @param {String} commentedCode the commented code
 * @return {String} the result of a String.match that returns the first non-comment - or null String if
 * commentedCode starts with a comment. If there are no comments then commentedCode is returned.
 */
function firstNonComment(commentedCode) {
    var match = commentedCode;
    if(commentedCode.indexOf("/**") !== -1) {
        match = commentedCode.match("([\\s\\S]*?)/\\*\\*");
